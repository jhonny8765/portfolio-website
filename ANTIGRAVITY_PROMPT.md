...
1. Map the app: App Router vs Pages, layout, global CSS, fonts, existing animation libs, contact form, Ask My AI entry point, `/playground`, project case-study routes.
2. List every current section and route. Do not delete routes or API handlers.
3. Confirm stack (expected: Next.js, React, Tailwind). Keep it.
4. Add only: `gsap` + `@gsap/react` + `ScrollTrigger` + `ScrollToPlugin` (and SplitText if the GSAP license in this project allows; otherwise a small custom split). Add `lenis` for smooth scroll. Do **not** add Framer Motion unless something already depends on it.
5. Prefer CSS for simple hovers. GSAP is for scroll, cursor, magnetic, preload, transitions, split text, trails.
...
8. **Mouse-follow / parallax illustrations in the hero + about**  
   4–7 small floating assets (console glyphs, `{AI}`, n8n nodes, circuit traces, a jeepney-or-Mindanao-subtle motif, a milk-tea cup, a bracket). They parallax on mouse move. Do **not** copy Ken’s cartoon character set. Generate or use original simple SVG/3D-ish assets in `/public`. Subtle, not childish.
...
3. **README / About** (personality + photo or stylized portrait if one exists in `/public`; otherwise a strong typographic monogram `JR` — do not use a random stock face).
...
- **Accent:** one electric signal color (suggested: amber/volt `#E8F54A` **or** teal `#3EE0C5` — pick one and commit). Use it for cursor ring, marquee separators, live pills, focus, terminal prompts.
