...
export async function POST(req: Request) {
  try {
    ...
    if (trimmedPrompt.length === 0 || trimmedPrompt.length > 500) {
      return new Response(JSON.stringify({ error: 'Prompt must be between 1 and 500 characters.' }), { status: 400 });
    }
    ...
    if (!accountId || !apiToken) {
      ...
      return new Response(JSON.stringify({ error: 'Image generation service is misconfigured.' }), { status: 500 });
    }
    ...
    try {
      ...
      if (!cloudflareRes.ok) {
        ...
        return new Response(JSON.stringify({ error: 'Image provider failed to generate the image. Please try a different prompt.' }), { status: 502 });
      }
      ...
      if (fetchError instanceof Error && fetchError.name === 'AbortError') {
        return new Response(JSON.stringify({ error: 'Request to image provider timed out.' }), { status: 504 });
      }
      throw fetchError;
    }
    ...
    return new Response(JSON.stringify({ error: 'An unexpected error occurred while generating the image.' }), { 
      status: 500,
